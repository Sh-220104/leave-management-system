...
public class AdminController {
    ...
    public ResponseEntity<?> adjustLeaveBalance(@PathVariable Long employeeId, @RequestParam Long leaveTypeId, @RequestParam Double amount) {
        adminService.adjustLeaveBalance(employeeId, leaveTypeId, amount);
        return ResponseEntity.ok().build();
    }

    @PostMapping("/role/{employeeId}")
    public ResponseEntity<?> setRole(@PathVariable Long employeeId, @RequestParam String role) {
        adminService.setRole(employeeId, role);
        return ResponseEntity.ok().build();
    }

    @PostMapping("/leave-type")
    public ResponseEntity<?> createLeaveType(@RequestParam String type, @RequestParam String description) {
